...
def test_admin_listar_produtos(mock_get_db, client):
    mock_db = MagicMock()
    mock_get_db.return_value = mock_db
    ...
    assert len(data) == 1
    assert data[0]["titulo"] == "Camiseta"
    assert data[0]["descricao"] == "Camiseta preta básica"
    assert data[0]["quantidade"] == 10
    assert data[0]["cor"] == "preto"
    assert data[0]["valor"] == 59.9
    assert data[0]["desconto"] == 10
    assert data[0]["tamanho"] == "M"
    assert "_id" not in data[0]


@patch("rotas.adm.get_db")
def test_admin_visualizar_produto(mock_get_db, client):
    mock_db = MagicMock()
    mock_get_db.return_value = mock_db
    ...
    data = response.get_json()
    assert data["titulo"] == "Camiseta"
    assert data["descricao"] == "Camiseta preta básica"
    assert data["quantidade"] == 10
    assert data["cor"] == "preto"
    assert data["valor"] == 59.9
    assert data["desconto"] == 10
